...
export class MapComponent {
  ...
  handleKeyboardEvent(event: KeyboardEvent) {
    ...
    switch (event.code) {
      case 'ArrowDown':
        if (this.map[currentPosition + 32] !== 1) this.heroMovementService.heroPosition$.next(currentPosition + 32);
        break;
      case 'ArrowUp':
        if (this.map[currentPosition - 32] !== 1) this.heroMovementService.heroPosition$.next(currentPosition - 32);
        break;
      case 'ArrowRight':
        if (this.map[currentPosition + 1] !== 1) this.heroMovementService.heroPosition$.next(currentPosition + 1);
        break;
      case 'ArrowLeft':
        if (this.map[currentPosition - 1] !== 1) this.heroMovementService.heroPosition$.next(currentPosition - 1);
        break;
